...
class ResolveAppPathsTests(unittest.TestCase):
    def test_macos_default_layout_collapses_to_dot_dir(self) -> None:
        with isolated_env("darwin"), TemporaryDirectory() as temp:
            ...
            self.assertEqual(paths.skills_store_root, base / "packages" / "local" / "skills")
            self.assertEqual(paths.skills_store_manifest, base / "packages" / "local" / "manifest.json")
    ...
    def test_xdg_overrides_each_dir_independently(self) -> None:
        with isolated_env("darwin"), TemporaryDirectory() as temp:
            ...
            self.assertEqual(paths.skills_store_root, root / "data" / APP_NAME / "packages" / "local" / "skills")
